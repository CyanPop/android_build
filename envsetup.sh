...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cyanpop_") ; then
       CYANPOP_BUILD=$(echo -n $1 | sed -e 's/^cyanpop_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $CYANPOP_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CYANPOP_BUILD=
    fi
    export CYANPOP_BUILD
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CYANPOP_VERSION)
        ZIPFILE=cyanpop-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.cyanpop.device | grep -q "$CYANPOP_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CYANPOP_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.cyanpop.device | grep -q "$CYANPOP_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CYANPOP_FIXUP_COMMON_OUT ]; then
